		...
		public void actionPerformed(ActionEvent e){
			...
			for(int i=0 ; i < textFieldList.size() ; i++){
				if(((JTextField) textFieldList.get(i)).getText().equals("")){
					JOptionPane.showMessageDialog(new JFrame(), "Ability brackets need to be filled.", "ERROR", JOptionPane.ERROR_MESSAGE);
					filled = false;
					break;
				}
				else{
			...
			if(filled==true){
				...
				if (duplicates==false){	
					for(int i=0 ; i < textFieldList.size(); i++){
						mainWindow.p.insert_member(Character.toString ((char) (65+i)), mainWindow.StringtoAbstraction(((JComboBox) cbList.get(i)).getSelectedItem().toString()), ((JTextField) textFieldList.get(i)).getText());
					}
					dispose();
					new ConnectionsWindow();
				}else{
					JOptionPane.showMessageDialog(new JFrame(), "Abilities need to be unique.", "ERROR", JOptionPane.ERROR_MESSAGE);
				}
			}
		}
	}

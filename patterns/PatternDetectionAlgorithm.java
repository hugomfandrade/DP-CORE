...
public class PatternDetectionAlgorithm {
	...
	private static Boolean ConnectionsCheck(ArrayList<ClassObject> co, ClassObject o, int depth){
		for( Connection ac : p.get_Connections()){
			Boolean flag = true;
			if (ac.get_From().getName().equalsIgnoreCase(Character.toString ((char) (65+depth))) && ((int) (ac.get_To().getName().toCharArray()[0])-65)<depth){
				switch (ac.get_type()) {
				case has:
			...
			else if(ac.get_To().getName().equalsIgnoreCase(Character.toString ((char) (65+depth))) && ((int) (ac.get_From().getName().toCharArray()[0])-65)<depth){
				switch (ac.get_type()) {
				case has:
